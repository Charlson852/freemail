  ...
  try {
    ...
    switch (currentBatchAction) {
      ...
      case 'delete-mailboxes':
        {
          const merged = Array.from(new Set([...emails, ...Array.from(selectedMailboxAddresses)]));
          if (!merged.length) { showToast('请先输入邮箱或先全选', 'error'); return; }
          if (!confirm(`确定批量删除这 ${merged.length} 个邮箱吗？这会同时删除关联邮件。`)) return;
          result = await batchDeleteMailboxes(merged);
          selectedMailboxAddresses.clear();
        }
        break;
    }
...
els.batchDeleteMailboxes?.addEventListener('click', () => openBatchModal('delete-mailboxes', '批量删除邮箱', '🗑️', '输入要删除的邮箱地址（每行一个、用逗号分隔，或先点击全选当前页）：'));
