  ...
  try {
    ...
    switch (currentBatchAction) {
      ...
      case 'delete-mailboxes':
        if (!confirm(`确定批量删除这 ${emails.length} 个邮箱吗？这会同时删除关联邮件。`)) return;
...
els.batchDeleteMailboxes?.addEventListener('click', () => openBatchModal('delete-mailboxes', '批量删除邮箱', '🗑️', '输入要删除的邮箱地址（每行一个或用逗号分隔）：'));
