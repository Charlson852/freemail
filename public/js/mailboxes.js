...
function bindCardEvents() {
  ...
  els.grid?.querySelectorAll('.mailbox-select').forEach(box => {
    const addr = String(box.dataset.mailboxSelect || '');
    box.checked = selectedMailboxAddresses.has(addr);
    box.addEventListener('change', (e) => {
      const value = String(e.target.dataset.mailboxSelect || '');
      if (!value) return;
      if (e.target.checked) selectedMailboxAddresses.add(value); else selectedMailboxAddresses.delete(value);
...
function getVisibleMailboxAddresses() {
  return currentData.map(item => String(item.address || '').trim().toLowerCase()).filter(Boolean);
}
  ...
  try {
    ...
    switch (currentBatchAction) {
      ...
      case 'clear-forward':
        ...
        break;
      case 'delete-mailboxes':
        {
          // 优先使用已勾选的邮箱地址
          const selected = Array.from(selectedMailboxAddresses);
          // 如果没勾选任何邮箱，才使用输入的邮箱地址
          const addressesToUse = selected.length > 0 ? selected : (emails || []);
          if (!addressesToUse.length) {
            showToast('请先勾选邮箱或输入邮箱地址', 'error');
            return;
          }
          if (!confirm(`确定批量删除这 ${addressesToUse.length} 个邮箱吗？这会同时删除关联邮件。`)) return;
          result = await batchDeleteMailboxes(addressesToUse);
          selectedMailboxAddresses.clear();
        }
        break;
    }
...
els.batchDeleteMailboxes?.addEventListener('click', () => openBatchModal('delete-mailboxes', '批量删除邮箱', '🗑️', '未选择了邮箱时，在此输入要删除的邮箱地址（每行一个或用逗号分隔）：'));
